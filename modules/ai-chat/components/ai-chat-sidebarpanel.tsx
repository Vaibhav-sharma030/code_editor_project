...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const getChatModePrompt = (mode: string, content: string) => {
    switch(mode){
        case "review":
            return `Please review this code and provide detailed suggestion for improvement, including performance, 
            security, and best practices:\n\n**Request:** ${content}`; 

        case "fix": 
            return `Please help fix issue in this code, including bugs, errors,
            and potential problems:\n\n**Problem:** ${content}`;

        case "optimize": 
            return `Please analyze this code for performance optimization ans suggest 
  ...
  const handleSendMessage = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    try {
        ...
        setMessages((prev) => [
            ...
            {
                role: "assistant",
                content: "I'm having trouble right now. Please check your internet connection and try again.",
                timestamp: new Date(),
                id: Date.now().toString(),
            },
        ]);
